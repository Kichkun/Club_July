...
void QVGA_Image(String title) {
  ...
  while (h--) {
    ...
    while (w--) {
      ...
      while ((PIND & 4));   //wait for low
      while (!(PIND & 4));  //wait for high
      //Serial.write(dataBuffer[319 - w]);
      //Wire.write(dataBuffer[319 - w]);
      // TODO Create valid types conversion (sigle byte -> char *)
      bus.send(14, {(char)dataBuffer[319 - w]}, 1);
    }
